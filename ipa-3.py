...
def tic_tac_toe(board):
    ...
    if len(set([board[row][row] for row in range(len(board))])) == 1 and board[0][0] != '':
        return board[0][0]

    if len(set([board[row][len(board)-1-row] for row in range(len(board))])) == 1 and board[0][len(board)-1] != '':
...
def eta(first_stop, second_stop, route_map):
    ...
    if (first_stop, second_stop) not in route_map:
        return "NO ROUTE"
    
    travel_time_mins = route_map[(first_stop, second_stop)]['travel_time_mins']
