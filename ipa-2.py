...
def shift_letter(letter, shift):
    ...
    *Note: the single underscore `_` is used to acknowledge the presence
    ...
    shifted_code = (shift + letter_code) % 26
...
def caesar_cipher(message, shift):
    ...
    for letter in message:
        if letter  == ' ':
            ...
            ascii_shifted = (ord(letter) - 65 + shift) % 26 + 65
            result += chr(ascii_shifted)
...
def shift_by_letter(letter, letter_shift):
    ...
    shifted_value = (shift_value + letter_value) % 26
...
def scytale_decipher(message, shift):
    ...
    grid_size = len(message) // shift
    message_grid = [[None]*grid_size for _ in range(shift)]
    message_index = 0

    for col in range(grid_size):
        for row in range(shift):
            message_grid[row][col] = message[message_index]
            message_index += 1

    decoded_message = ''
    for row in range(shift):
        for col in range(grid_size):
            decoded_message += message_grid[row][col]
